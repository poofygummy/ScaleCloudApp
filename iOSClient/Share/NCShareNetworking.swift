...
class NCShareNetworking: NSObject {
    ...
    private func readDownloadLimit(account: String, token: String) async throws -> SCKDownloadLimit? {
        return try await withCheckedThrowingContinuation { continuation in
            SCKClient.shared.getDownloadLimit(account: account, token: token) { limit, error in
                if error != .success {
    ...
    func readShare(showLoadingIndicator: Bool) {
        ...
        SCKClient.shared.readShares(parameters: parameter, account: metadata.account) { task in
            Task {
            ...
            if error == .success, let shares = shares {
                ...
                SCKClient.shared.getGroupfolders(account: account) { task in
                    Task {
    ...
    func createShare(_ shareable: Shareable, downloadLimit: DownloadLimitViewModel) {
        ...
        SCKClient.shared.createShare(path: filenamePath,
    ...
    func unShare(idShare: Int) {
        NCActivityIndicator.shared.start(backgroundView: view)
        SCKClient.shared.deleteShare(idShare: idShare, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: self.metadata.account,
                                                                                            path: "_\(idShare)",
                                                                                            name: "deleteShare")
    ...
    func updateShare(_ shareable: Shareable, downloadLimit: DownloadLimitViewModel, changeDownloadLimit: Bool = false) {
        NCActivityIndicator.shared.start(backgroundView: view)
        SCKClient.shared.updateShare(idShare: shareable.idShare, password: shareable.password, expireDate: shareable.formattedDateString, permissions: shareable.permissions, note: shareable.note, label: shareable.label, hideDownload: shareable.hideDownload, attributes: shareable.attributes, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: self.metadata.account,
    ...
    func getSharees(searchString: String) {
        NCActivityIndicator.shared.start(backgroundView: view)
        SCKClient.shared.searchSharees(search: searchString, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: self.metadata.account,
                                                                                            path: searchString,
                                                                                            name: "searchSharees")
    ...
    func removeShareDownloadLimit(token: String) {
        ...
        SCKClient.shared.removeShareDownloadLimit(account: metadata.account, token: token) { error in
            NCActivityIndicator.shared.stop()
    ...
    func setShareDownloadLimit(_ limit: Int, token: String) {
        ...
        SCKClient.shared.setShareDownloadLimit(account: metadata.account, token: token, limit: limit) { error in
            NCActivityIndicator.shared.stop()
