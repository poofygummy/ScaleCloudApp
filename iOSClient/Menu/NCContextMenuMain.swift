...
class NCContextMenuMain: NSObject {
    ...
    private func makeUnsetFolderE2EEAction(metadata: tableMetadata) -> UIAction {
        return UIAction(
            ...
            Task {
                let results = await SCKClient.shared.markE2EEFolderAsync(
                    ...
                    let deferredElement = UIDeferredMenuElement { completion in
                        Task {
                            ...
                            if let iconUrl = item.icon {
                                let results = await SCKClient.shared.downloadContentAsync(serverUrl: metadata.urlBase + iconUrl, account: metadata.account)
                            ...
                            let action = UIAction(
                                ...
                                Task {
                                    let results = await SCKClient.shared.sendRequestAsync(
                                        account: metadata.account,
                                        fileId: metadata.fileId,
