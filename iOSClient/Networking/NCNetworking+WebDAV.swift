...
extension NCNetworking {
    ...
    func readFolderAsync(serverUrl: String,
        ...
        let resultsReadFolder = await SCKClient.shared.readFileOrFolderAsync(serverUrlFileName: serverUrl, depth: "1", showHiddenFiles: showHiddenFiles, account: account, options: options) { task in
            Task {
    ...
    func readFile(serverUrlFileName: String,
        ...
        SCKClient.shared.readFileOrFolder(serverUrlFileName: serverUrlFileName, depth: "0", showHiddenFiles: showHiddenFiles, account: account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: account,
    ...
    func readFileAsync(serverUrlFileName: String,
        ...
        let showHiddenFiles = NCPreferences().getShowHiddenFiles(account: account)
        let results = await SCKClient.shared.readFileOrFolderAsync(serverUrlFileName: serverUrlFileName,
                                                                      depth: "0",
    ...
    func fileExists(serverUrlFileName: String, account: String) async -> SCKError {
        let requestBody = SCKDataFileXML(nkCommonInstance: SCKClient.shared.nkCommonInstance).getRequestBodyFileExists().data(using: .utf8)
    ...
    func createFolder(fileName: String,
        ...
        let resultCreateFolder = await SCKClient.shared.createFolderAsync(serverUrlFileName: serverUrlFileName, account: session.account, options: options) { task in
            Task {
    ...
    func createFolderForAutoUpload(serverUrlFileName: String, account: String) async -> SCKError {
        ...
        // Try to create the directory
        let results = await SCKClient.shared.createFolderAsync(serverUrlFileName: serverUrlFileName, account: account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: account,
                                                                                            path: serverUrlFileName,
                                                                                            name: "createFolder")
    ...
    func deleteFileOrFolder(metadata: tableMetadata) async -> SCKError {
        var results = await SCKClient.shared.deleteFileOrFolderAsync(serverUrlFileName: metadata.serverUrlFileName, account: metadata.account) { task in
            Task {
    ...
    func renameFileOrFolder(metadata: tableMetadata) async -> SCKError {
        ...
        let results = await SCKClient.shared.moveFileOrFolderAsync(serverUrlFileNameSource: serverUrlFileNameSource, serverUrlFileNameDestination: serverUrlFileNameDestination, overwrite: false, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: metadata.account,
    ...
    func moveFileOrFolder(metadata: tableMetadata) async -> SCKError {
        ...
        let results = await SCKClient.shared.moveFileOrFolderAsync(serverUrlFileNameSource: metadata.serverUrlFileName, serverUrlFileNameDestination: serverUrlFileNameDestination, overwrite: overwrite, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: metadata.account,
    ...
    func copyFileOrFolder(metadata: tableMetadata) async -> SCKError {
        ...
        let results = await SCKClient.shared.copyFileOrFolderAsync(serverUrlFileNameSource: metadata.serverUrlFileName, serverUrlFileNameDestination: serverUrlFileNameDestination, overwrite: overwrite, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: metadata.account,
                                                                                            path: serverUrlFileNameDestination,
    ...
    func setFavorite(metadata: tableMetadata) async -> SCKError {
        ...
        let results = await SCKClient.shared.setFavoriteAsync(fileName: fileName, favorite: metadata.favorite, account: metadata.account) { task in
            Task {
                let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: metadata.account,
                                                                                            path: fileName,
                                                                                            name: "setFavorite")
    ...
    func lockUnlockFile(_ metadata: tableMetadata, shouldLock: Bool) async -> SCKError {
        do {
            _ = try await SCKClient.shared.lockUnlockFile(serverUrlFileName: metadata.serverUrlFileName, shouldLock: shouldLock, account: metadata.account)
    ...
    func getVideoUrl(metadata: tableMetadata,
        ...
        if !metadata.url.isEmpty {
            ...
            SCKClient.shared.getDirectDownload(fileId: metadata.fileId, account: metadata.account) { task in
                Task {
                    let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: metadata.account,
...
class NCOperationDownloadAvatar: ConcurrentOperation, @unchecked Sendable {
    ...
    override func start() {
        ...
        SCKClient.shared.downloadAvatar(user: user,
                                           fileNameLocalPath: fileNameLocalPath,
                                           sizeImage: NCGlobal.shared.avatarSize,
                                           avatarSizeRounded: NCGlobal.shared.avatarSizeRounded,
                                           etagResource: self.etag,
                                           account: account,
                                           options: SCKRequestOptions(queue: SCKClient.shared.nkCommonInstance.backgroundQueue)) { task in
            Task {
