...
extension NCNetworking {
    ...
    func cancelTask(metadata: tableMetadata) async {
        ...
        default:
            ...
            // UPLOAD
                ...
                if metadata.session == SCKClient.shared.nkCommonInstance.identifierSessionUpload {
    ...
    func cancelAllDataTask() {
        SCKClient.shared.nkCommonInstance.nksessions.forEach { session in
    ...
    func cancelUploadTasks(metadata: tableMetadata? = nil) {
        ...
        Task {
            SCKClient.shared.nkCommonInstance.nksessions.forEach { nkSession in
    ...
    func getAllDataTask() async -> [URLSessionDataTask] {
        let nkSessions = SCKClient.shared.nkCommonInstance.nksessions.all
    ...
    func verifyZombie() async {
        ...
        if let metadatas = await NCManageDatabase.shared.getMetadatasAsync(
            predicate: NSPredicate(format: "session == %@ AND status == %d",
                                   ...
                                   self.global.metadataStatusUploading)) {
            for metadata in metadatas {
                guard let nkSession = SCKClient.shared.nkCommonInstance.nksessions.session(forAccount: metadata.account) else {
                    await NCManageDatabase.shared.deleteMetadataAsync(id: metadata.ocId)
        ...
        if let metadatas = await NCManageDatabase.shared.getMetadatasAsync(
            predicate: NSPredicate(format: "(session == %@ OR session == %@) AND status == %d",
                                   ...
                                   self.global.metadataStatusUploading)) {
            for metadata in metadatas {
                guard var nkSession = SCKClient.shared.nkCommonInstance.nksessions.session(forAccount: metadata.account) else {
                    await NCManageDatabase.shared.deleteMetadataAsync(id: metadata.ocId)
                    utilityFileSystem.removeFile(atPath: utilityFileSystem.getDirectoryProviderStorageOcId(metadata.ocId,
                                                                                                           userId: metadata.userId,
                                                                                                           urlBase: metadata.urlBase))
