...
class NCMediaDownloadThumbnail: ConcurrentOperation, @unchecked Sendable {
    ...
    override func start() {
       Task {
           ...
           let resultsDownloadPreview = await SCKClient.shared.downloadPreviewAsync(fileId: tblMetadata.fileId, etag: tblMetadata.etag, account: tblMetadata.account, options: SCKRequestOptions(queue: SCKClient.shared.nkCommonInstance.backgroundQueue)) { task in
               Task {
