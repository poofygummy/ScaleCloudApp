...
class NCCreate: NSObject {
    ...
    func createDocument(controller: NCMainTabBarController, fileNamePath: String, fileName: String, editorId: String, creatorId: String? = nil, templateId: String, account: String) async {
        ...
        if let creatorId, let adapter = NCDirectEditorAdapter.resolve(from: [editorId]) {
            ...
            let results = await SCKClient.shared.textCreateFileAsync(fileNamePath: fileNamePath, editorId: editorId, creatorId: creatorId, templateId: templateId, account: account, options: options) { task in
                Task {
            ...
            let results = await SCKClient.shared.createRichdocumentsAsync(path: fileNamePath, templateId: templateId, account: account) { task in
                Task {
                    let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: account,
                                                                                                path: fileNamePath,
    ...
    func getTemplate(editorId: String, templateId: String, account: String) async -> (templates: [SCKEditorTemplate], selectedTemplate: SCKEditorTemplate, ext: String) {
        ...
        if let adapter = NCDirectEditorAdapter.resolve(from: [editorId]) {
            ...
            let results = await SCKClient.shared.textGetListOfTemplatesAsync(account: account, options: options) { task in
                Task {
                    let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: account,
        ...
        if editorId == "collabora" {
            let results = await SCKClient.shared.getTemplatesRichdocumentsAsync(typeTemplate: templateId, account: account) { task in
                Task {
                    let identifier = await NCNetworking.shared.networkingTasks.createIdentifier(account: account,
                                                                                                path: templateId,
