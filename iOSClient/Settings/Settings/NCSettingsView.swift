...
struct NCDebugView: View {
    ...
    private func signingDiagnostics() -> String {
        ...
        let anisetteURL = UserDefaults.standard.string(forKey: "menuAnisetteURL") ?? ""
        if !anisetteServers.isEmpty {
            parts.append("anisette:\(anisetteURL.isEmpty ? "✓(no url)" : anisetteURL)")
        } else {
            parts.append("anisette:✗(empty list)")
        }
    ...
    private func dumpSigningUserDefaults() {
        ...
        let signCredentialsInjected = ud.bool(forKey: "com.scalecloud.credentialsInjected")
        ...
        nkLog(debug: "[UserDefaults] signCredentialsInjected: \(signCredentialsInjected)")
